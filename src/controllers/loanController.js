...
const createLoan = AsyncHandler(async (req, res) => {
    ...
    if (!emiAmount ) {
        return res.status(400).json(
            new ApiResponse(400, "Loan schedule details are required 2")
        );
    }
    if (!numberOfEmis) {
        return res.status(400).json(
            new ApiResponse(400, "Loan schedule details are required 3")
        );
    }
    if (!emiDueDate) {
        return res.status(400).json(
            new ApiResponse(400, "Loan schedule details are required 4")
        );
    }
